...
@section('content')
        ...
        <tbody>
            @foreach ($rooms as $room)
                <tr id="row_{{ $room->id }}">
                    <td>{{ $room->id }}</td>
                    <td>{{ $room->name }}</td>
                    <td>{{ $room->descripitoin }}</td>
                    <td>{{ $room->roomType->id }}</td>
                    <td>{{ $room->hotel->id }}</td>
                    <td class="action-column">
                    <a onclick="updatePost(event)" data-toggle="modal" data-target="#EidtPost20"
                        data-name="{{ $room->name }}"
                        data-descripitoin="{{ $room->descripitoin }}"
                        data-type="{{ $room->roomType->id  }}"
                        data-hotel="{{ $room->hotel->id  }}"
                        href="{{ route('admin.rooms.update', $room->id) }}" class="btn btn-primary btn-sm">
                        <i class="fas fa-edit"></i>
                    </a>
                        <form class="d-inline" action="{{ route('admin.rooms.destroy', $room->id) }}" method="POST">
                            @csrf
                            @method('delete')
                    ...
                    {{-- <a
                        href="{{ route('admin.rooms.show', $room->id) }}" class="btn btn-primary btn-sm">
    ...
    <a  class="custom-button" href="{{ route('admin.rooms.pdf','v') }}">pdf</a>
    <!-- Modal -->
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Eidt Room.</h5>
    ...
    <script>
     ...
     <script>
        function updatePost(e) {
            ...
            let hotel = e.target.closest('a').dataset.hotel;
            ...
            document.querySelector('[name=hotel]').value = hotel;
        ...
        function editPost(e) {
            ...
            axios.post(url, data)
                .then(res => {
                    ...
                    $('#row_' + res.data.id + ' td:nth-child(3)').html(res.data.descripitoin)
                    $('#row_' + res.data.id + ' td:nth-child(4)').html(res.data.type)
                    $('#row_' + res.data.id + ' td:nth-child(5)').html(res.data.hotel)
