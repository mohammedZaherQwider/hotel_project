                ...
                <tr id="row_{{ $reservation->id }}">
                    ...
                    <td class="action-column">
                        <a onclick="updatePost(event)" data-toggle="modal" data-target="#EidtPost20"
                            data-name="{{ $reservation->check_out }}"
                            href="{{ route('admin.reservations.update', $reservation->id) }}" class="btn btn-primary btn-sm">
                            <i class="fas fa-edit"></i>
                        </a>
                        <form class="d-inline" action="{{ route('admin.reservations.destroy', $reservation->id) }}" method="POST">
                            @csrf
                            @method('delete')
...
<a  class="custom-button" href="{{ route('admin.reservations.pdf','v') }}">pdf</a>
    <!-- Modal -->
...
@section('js')
    ...
    <script>
        function updatePost(e) {
            ...
            let check_out = e.target.closest('a').dataset.check_out;
            let url = e.target.closest('a').href;

            // show old data in edit form
            document.querySelector('[name=check_out]').value = check_out;
        ...
        function editPost(e) {
            ...
            axios.post(url, data)
                .then(res => {
                    $('#row_' + res.data.id + ' td:nth-child(3)').html(res.data.check_out)
